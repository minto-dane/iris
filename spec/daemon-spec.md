...
3. 初期実装は TCP / UDP / HTTP listen を提供してはならない MUST NOT。
4. 1 connection は 1 request を処理 MUST とする。
5. 初期 daemon は単一 frontend 経路を主対象とし、同時多重 client 処理を保証しない MAY。
...
3. 初期実装では `--user <name|uid>` を privilege drop target 指定として提供 SHOULD とする。
...
2. status file 更新では temp file への書き込み、flush / sync、rename、親 directory の sync を行う SHOULD。
3. daemon は各 verify 実行結果を `<state-root>/log/daemon-verify.jsonl` に追記 SHOULD する。
4. verify log 追記後は flush / sync を行い、部分書き込みを黙殺してはならない MUST NOT。
5. status record は少なくとも以下を含む SHOULD:
   ...
   - `issue_count`
   - `report` または `error`
6. status artifact の書き込み失敗は daemon 実装上のエラーとして扱い、黙殺してはならない MUST NOT。

## 13. daemon runtime options
