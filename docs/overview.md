...
初期作成時から owner-private permission で扱う。
...
10. daemon status / log の読み出しは read-only かつ bounded に扱い、no-follow / post-open 検証を優先し、欠損や破損は明示的エラーまたは空結果として返す。
11. daemon status / verify log の書き込みは temp file / atomic rename / parent dir sync / bounded retention を用いて hardening する。
12. `run` / `log` / `tmp` のような sensitive state directory は初期 layout 作成時から owner-private permission に正規化する。
